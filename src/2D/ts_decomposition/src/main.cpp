...
std::vector<double> talpha(IMAX * JMAX);
std::vector<double> tbeta(IMAX * JMAX);
...
inline void BaseTrapezoid(int t0, int t1,
  ...
  for (int ts = t0; ts < t1; ++ts) {
    for (int j = j0; j < j1; ++j) {
      for (int i = i0; i < i1; ++i) {
        STATE_NEXT[INDEX2D(i, j)] = STATE_NOW[INDEX2D(i, j)] + (nu * dt / (dx * dx)) * (STATE_NOW[INDEX2D(i-1, j)] - 2.0* STATE_NOW[INDEX2D(i, j)] + STATE_NOW[INDEX2D(i+1, j)])
                                                             + (nu * dt / (dy * dy)) * (STATE_NOW[INDEX2D(i, j-1)] - 2.0* STATE_NOW[INDEX2D(i, j)] + STATE_NOW[INDEX2D(i, j+1)]);
        if (i==1) {
          STATE_NEXT[INDEX2D(i-1, j)] = STATE_NEXT[INDEX2D(i, j)];
        } else if (i == IMAX - 2) {
          STATE_NEXT[INDEX2D(i+1, j)] = STATE_NEXT[INDEX2D(i, j)];
        } 
        if (j == 1) {
          STATE_NEXT[INDEX2D(i, j-1)] = STATE_NEXT[INDEX2D(i, j)];
        } else if (j == JMAX - 2) {
          STATE_NEXT[INDEX2D(i, j+1)] = STATE_NEXT[INDEX2D(i, j)];
        } 
      }
...
inline void RecursiveTrapezoid(int t0, int t1,
  ...
  if (t_span > 1) { // Trapezoid slope becomes meaningless over a single timestep
    if (i_span > i_threshold && i_span >= j_span && i_span > split_threshold) {
    ...
    if (j_span > j_threshold && j_span > split_threshold) {
  ...
  BaseTrapezoid(t0, t1, i0, del_i0, i1, del_i1, j0, del_j0, j1, del_j1);
}
...
int main() {
  for (int j = 0; j < JMAX; ++j) {
    for (int i = 0; i < IMAX; ++i) {
      if (i < HOTCORNER_IMAX && j < HOTCORNER_JMAX) {
        STATE_INITIAL[INDEX2D(i, j)] = 2.0;
      } else {
        STATE_INITIAL[INDEX2D(i, j)] = 1.0;
      }
    }
  }
  ...
  for (int j = 1; j < JMAX-1; ++j) {
    temperature = std::accumulate(&STATE_INITIAL[INDEX2D(1, j)], &STATE_INITIAL[INDEX2D(IMAX-1, j)], temperature);
  }
  std::cout << "Initial Temperature: " << temperature << " Expected: " << expected << std::endl;
  RecursiveTrapezoid(0, TIMESTEPS, 1, 0, IMAX-1, 0, 1, 0, JMAX-1, 0);
  temperature = 0.0;
  for (int j = 1; j < JMAX-1; ++j) {
    temperature = std::accumulate(&STATE_FINAL[INDEX2D(1, j)], &STATE_FINAL[INDEX2D(IMAX-1, j)], temperature);
  }
