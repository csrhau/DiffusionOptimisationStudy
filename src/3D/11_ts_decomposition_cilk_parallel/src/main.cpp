...
#define STATE_INITIAL talpha.data()
...
inline void RecursiveTrapezoid(int t0, int t1,
                               ...
                               int j0, int del_j0, int j1, int del_j1,
                               int k0, int del_k0, int k1, int del_k1) {
  if ( (i0 == i1 && del_i0 == del_i1)
    || (i0 == i1 && del_i0 == del_i1)
    || (i0 == i1 && del_i0 == del_i1) ) {
    // Empty Trapezoid, early return
    return;
  }
  int t_span = t1 - t0;
  int i_span = i1 - i0;
  ...
  if (t_span > 1) { // Trapezoid slope becomes meaningless over a single timestep
    if (i_span > i_threshold && i_span >= j_span && i_span >= k_span && i_span > split_threshold) {
      ...
      // Core trapezoids (x2)
      cilk_spawn RecursiveTrapezoid(t0, t1,
                                    i0, STENCIL_RADIUS,
...
int main() {
  ...
  cilk_for (int k = 1; k < KMAX-1; ++k) {
    for (int j = 1; j < JMAX-1; ++j) {
      ...
      *temperature_initial += __sec_reduce_add(STATE_INITIAL[start:length]);
    }
  }
  ...
  cilk_for (int k = 1; k < KMAX-1; ++k) {
    for (int j = 1; j < JMAX-1; ++j) {
      const unsigned int start = INDEX3D(1, j, k);
      const unsigned int length = IMAX-2; 
      *temperature_final += __sec_reduce_add(STATE_FINAL[start:length]);
    }
  }
